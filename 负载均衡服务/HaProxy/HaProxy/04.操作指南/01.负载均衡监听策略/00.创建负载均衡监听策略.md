...
3. 在监听策略创建页，配置 TCP 或 HTTP/HTTPS 策略。
   ...
   <table width="95%" border="1" cellpadding="2" cellspacing="1">
       ...
       <tbody>
   		...
   		<tr>
               <td>DNS 配置</td>
               <td>支持选择配置 DNS，默认关闭，当后端服务器为域名时，配置后可对域名进行健康检测。未开启 DNS 时，须确保域名与负载均衡实例网络互通。</td>
           </tr>
   		<tr>
               <td>访问控制</td>
   ...
   >**DNS配置**:
   >DNS配置添加DNS详情参见 [DNS管理](./04.DNS管理.md)
   ...
   | 接受无效HTTP请求 | 默认关闭，开启后负载均衡实例将接受无效的 HTTP 请求。         |
...
2. 在策略配置页面，点击右上角 **批量添加策略**。

   ![策略列表](../../pic/batch-rule1.png)

3. 在批量添加监听策略页，可以点击左下角 **增加单条策略** 以批量编辑监听策略，在列表项右侧可查看格式要求。

   ![策略列表](../../pic/batch-rule5.png)

4. 您也可以通过文件方式导入策略，点击左上角 **批量导入策略**，在弹窗中下载模版后根据模版内格式录入数据，选择该文件导入即可。

   ![策略列表](../../pic/batch-rule4.png)

5. 配置完成检查无误后，点击 **批量添加**。

   ![策略列表](../../pic/batch-rule2.png)

6. 在批量添加策略弹窗中，点击 **确认** 即可批量添加策略。

   ![策略列表](../../pic/batch-rule3.png)

7. 批量添加策略完成后，您可通过点击监听策略列表操作中 **配置** 修改监听策略信息。
